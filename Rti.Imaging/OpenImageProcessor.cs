...
namespace Rti.Imaging
{
	...
	/// </summary>
	public class OpenImageProcessor
	{
		#region Globals
            ...
            int height)
		{
            ...
            if (width == 0 && height == 0)
            {
			    pageSize.Native = Settings.GhostscriptPageSizes.a7;
            }
            else
            {
        ...
        /// <summary>
        /// Compatibility with old processor calling convention
        /// </summary>
	    public static void convertPDF2TIF(string pdfFile, string tiffFileOut, int intBits, 
            ref int numPages, ref string outMessage)
	    {
            ...
            GenerateOutput(pdfFile, tiffFileOut, settings);

            if (System.IO.File.Exists(tiffFileOut))
            {
                numPages = getTIFFPageCount(tiffFileOut);
                outMessage = "";
            }
            else
            {
                outMessage = "GhostScript failed to create TIF file, file not found";
            }
	    }

	    private static int getTIFFPageCount(string istrSrcFile)
	    {
	        return 0;
	    }
...
namespace Rti.Imaging.Settings
{
    public class GhostScriptCompression
    {
